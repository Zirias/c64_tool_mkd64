 ...
 * @param tracknum the number of self track on the disk image
 * @param num_sectors the number of sectors to use on self track
 * @return a new track instance
 */
DECLEXPORT Track *Track_init(Track *self, int tracknum, size_t num_sectors);

/** Delete a track instance
 * self frees all memory allocated by self track and the blocks contained in it
...
/** Get number of sectors on self track
 * @param self the track
 * @return the number of sectors
 */
DECLEXPORT size_t Track_numSectors(const Track *self);

/** Get number of free sectors on self track
 * @param self the track
 ...
 * self is a convenience function to find and allocate the next available
